...
4. **Move the Downloaded FIPS/Commerical Bundle**
   ...
   ```
    ...
    Each product that has commerical support has there own respective directory structures to place there bundles.
...
5. **Edit *-details/wolfssl_%.bbappend**
    ...
    `WOLF_SRC_SHA = "<SHA_HASH>"`: `<SHA_HASH>` This is the sha hash given when you recived the bundled.
    `WOLF_SRC_PASS = "<PASSWORD>"`: `<PASSWORD>` This is the password given to unarchive the bundle.
    `WOLF_SRC = "<BUNDLE_NAME>"`: `<BUNDLE_NAME>` This is the name of the bundle you wish to use without the .7zp extension.  
