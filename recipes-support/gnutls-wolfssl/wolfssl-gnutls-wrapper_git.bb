...
DEPENDS = "wolfssl gnutls"
RDEPENDS:${PN} += "wolfssl gnutls"
...
do_install() {
    ...
    install -d ${D}${libdir}/gnutls
    if [ -f ${S}/libgnutls-wolfssl-wrapper.so ]; then
        install -m 0755 ${S}/libgnutls-wolfssl-wrapper.so ${D}${libdir}/gnutls/
        bbnote "Installed wrapper to ${D}${libdir}/gnutls/"
    else
    ...
    ln -sf ${libdir}/gnutls/libgnutls-wolfssl-wrapper.so \
